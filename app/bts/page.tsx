...
export default async function BTS() {
  ...
  const data = await batchFetchSanityData({
    btsSections: {
      query: '*[_type == "btsSection"] | order(orderRank asc) { _id, title, subtitle, layout, backgroundColor, mainImage, videoUrl, images[] { _key, image, alt } }'
    }
  })
      ...
      <div className="relative z-10">
        ...
        <main className="container mx-auto px-4 sm:px-6 lg:px-8 pt-24">
          ...
          <div className="mb-12 fade-in">
            <h1 className="text-4xl lg:text-5xl font-bold mb-2 slide-up">Be High The Scene</h1>
            <p className="text-gray-400 text-lg slide-up delay-200">A closer perspective</p>
          </div>
              ...
              {btsSections.map((section, index) => (
                <div key={section._id} className="mb-16 fade-in" style={{ animationDelay: `${index * 200}ms` }}>
                  ...
                  <div className="text-center mb-8 slide-up">
                    <h2 className="text-3xl lg:text-4xl font-light">{section.title || `BTS Session ${index + 1}`}</h2>
                  ...
                  {section.layout === 'layout1' && (
                    <div className={`rounded-2xl p-6 lg:p-8 mb-12 ${section.backgroundColor ? '' : 'bg-[#1a1a1a]'}`} style={section.backgroundColor ? { backgroundColor: section.backgroundColor.hex } : {}}>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                        ...
                        <div className="lg:col-span-2 grid grid-cols-2 gap-6">
                          {section.images && section.images.slice(0, 4).map((img: any, i: number) => {
                            ...
                            return (
                              <div key={img?._key || `layout1-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                                <Image
                          ...
                          {section.images && section.images.length < 4 && Array.from({ length: 4 - (section.images.length || 0) }).map((_, i) => (
                            <div key={`fallback-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                              <Image
                  ...
                  {section.layout === 'layout2' && (
                    <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12 ${section.backgroundColor ? 'p-6 lg:p-8 rounded-2xl' : ''}`} style={section.backgroundColor ? { backgroundColor: section.backgroundColor.hex } : {}}>
                      {/* Left side - 3 images */}
                      <div className="grid grid-cols-3 gap-4">
                        {section.images && section.images.slice(0, 3).map((img: any, i: number) => {
                          ...
                          return (
                            <div key={img?._key || `layout2-left-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                              <Image
                        ...
                        {section.images && section.images.length < 3 && Array.from({ length: 3 - (section.images.length || 0) }).map((_, i) => (
                          <div key={`fallback-left-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                            <Image
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {section.images && section.images.slice(3, 5).map((img: any, i: number) => {
                          ...
                          return (
                            <div key={img?._key || `layout2-right-${i}`} className="relative aspect-video rounded-lg overflow-hidden bg-gray-900">
                              <Image
                        ...
                        {section.images && (section.images.length < 5) && Array.from({ length: 2 - (Math.max(0, section.images.length - 3)) }).map((_, i) => (
                          <div key={`fallback-right-${i}`} className="relative aspect-video rounded-lg overflow-hidden bg-gray-900">
                            <Image
                  ...
                  {section.layout === 'layout3' && (
                    <div className={`grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-12 ${section.backgroundColor ? 'p-6 lg:p-8 rounded-2xl' : ''}`} style={section.backgroundColor ? { backgroundColor: section.backgroundColor.hex } : {}}>
                      {section.images && section.images.slice(0, 5).map((img: any, i: number) => {
                        // Kiểm tra xem img có hợp lệ không
                        const validImage = img && (img.image?.asset || img.asset || (typeof img === 'object' && Object.keys(img).length > 0 && img._key))

                        return (
                          <div key={img?._key || `layout3-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                            <Image
                              src={validImage ? urlFor(img.image || img).width(300).height(300).url() : '/placeholder.svg?height=300&width=300'}
                              alt={img?.alt || `Gallery image ${i + 1}`}
                              fill
                      ...
                      {section.images && section.images.length < 5 && Array.from({ length: 5 - (section.images.length || 0) }).map((_, i) => (
                        <div key={`fallback-row-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                          <Image
                            src="/placeholder.svg?height=300&width=300"
                            alt={`Gallery image placeholder ${i + 1}`}
                            fill
                  ...
                  {section.layout === 'layout4' && (
                    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12 ${section.backgroundColor ? 'p-6 lg:p-8 rounded-2xl' : ''}`} style={section.backgroundColor ? { backgroundColor: section.backgroundColor.hex } : {}}>
                      {/* 2 hình lớn */}
                      <div className="lg:col-span-2 grid grid-cols-2 gap-6">
                        {section.images && section.images.slice(0, 2).map((img: any, i: number) => {
                          ...
                          return (
                            <div key={img?._key || `layout4-large-${i}`} className="relative aspect-video rounded-lg overflow-hidden bg-gray-900">
                              <Image
                        ...
                        {section.images && section.images.length < 2 && Array.from({ length: 2 - (section.images.length || 0) }).map((_, i) => (
                          <div key={`fallback-large-${i}`} className="relative aspect-video rounded-lg overflow-hidden bg-gray-900">
                            <Image
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {section.images && section.images.slice(2, 5).map((img: any, i: number) => {
                          ...
                          return (
                            <div key={img?._key || `layout4-small-${i}`} className="relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-900">
                              <Image
                        ...
                        {section.images && (section.images.length < 5) && Array.from({ length: 3 - (Math.max(0, section.images.length - 2)) }).map((_, i) => (
                          <div key={`fallback-small-${i}`} className="relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-900">
                            <Image
                  ...
                  {!section.layout && (
                    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12 ${section.backgroundColor ? 'p-6 lg:p-8 rounded-2xl' : ''}`} style={section.backgroundColor ? { backgroundColor: section.backgroundColor.hex } : {}}>
                      {section.images && section.images.map((img: any, i: number) => {
                        // Kiểm tra xem img có hợp lệ không
                        const validImage = img && (img.image?.asset || img.asset || (typeof img === 'object' && Object.keys(img).length > 0 && img._key))

                        return (
                          <div key={img?._key || `default-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                            <Image
                              src={validImage ? urlFor(img.image || img).width(400).height(400).url() : '/placeholder.svg?height=400&width=400'}
                              alt={img?.alt || `BTS image ${i + 1}`}
                              fill
                      ...
                      {(!section.images || section.images.length === 0) && Array.from({ length: 6 }).map((_, i) => (
                        <div key={`fallback-default-${i}`} className="relative aspect-square rounded-lg overflow-hidden bg-gray-900">
                          <Image
                            src="/placeholder.svg?height=400&width=400"
                            alt={`BTS image placeholder ${i + 1}`}
                            fill
